- 2026-02-04: UI V3: ensured “Déconnexion” always hard-navigates to `wp_logout_url()` (no AJAX/confirmation interference) and redirects back to `/user-account`; Orders empty-state CTA renamed to “Parcourir les produits” and points to `/produit/`. Files: custom code ilocker/ilocker custom code v3/user-interface-v3/il-user-interface.php, custom code ilocker/ilocker custom code v3/user-interface-v3/il-user-interface.js.
- 2026-02-04: Dashboard view: replaced default dashboard intro content with Elementor template output via `[elementor-template id="5683"]` (falls back to legacy content if shortcode isn’t available). Files: custom code ilocker/ilocker custom code v3/user-interface-v3/il-user-interface.php.
- 2026-02-03: Guest product login widget: moved card styling (stroke + shadow) into custom code ilocker/ilocker custom code v3/ilocker-css-v3.css; removed duplicate inline banner styling from hide woocommerce prices.php. Button keeps theme default (.button).
...
- UI polish: ensured the guest login widget is positioned right after the product title and styled as a card (stroke + shadow).
